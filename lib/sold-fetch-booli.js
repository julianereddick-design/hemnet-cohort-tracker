...
// ---------------------------------------------------------------------------
// fetchBooliDetail — fetch a single Booli sold detail page. Accepts EITHER a bare
// residenceId (digits → /bostad/<id>, backward-compatible) OR a card URL/path in
// either Booli flavor: "/bostad/<residenceId>" OR "/annons/<booliId>". Booli serves
// sold cards under BOTH forms (~half each) and BOTH detail pages carry the same
// SoldProperty node with `rent` — so keying only on /bostad/ silently drops the fee
// for /annons cards (apartment match-rate bug; see reference_booli_hemnet_sold_schema).
// Returns the parsed detail object or null (never throws; re-throws CeilingError).
// ---------------------------------------------------------------------------
async function fetchBooliDetail(idOrUrl, opts = {}) {
  const log = opts.logger || noop;
  if (!idOrUrl) return null;

  const s = String(idOrUrl);
  const url = s.startsWith('http') ? s
    : s.startsWith('/') ? `https://www.booli.se${s}`
      : `https://www.booli.se/bostad/${s}`;
  let res;
  try {
    res = await cachedFetch(url, { logger: log });
  } catch (e) {
    if (e instanceof CeilingError) throw e;
    log('WARN', `fetchBooliDetail ${url} fetch failed: ${e.message}`);
    return null;
  }

  if (res.status !== 200) {
    log('WARN', `fetchBooliDetail ${url} status ${res.status}`);
    return null;
  }
  ...
  try {
    ...
    log('WARN', `fetchBooliDetail ${url} apollo parse failed: ${e.message}`);
    return null;
  }
...
// NOTE: returns null for "/annons/<booliId>" cards — use extractDetailUrl when you
// need the fee for BOTH flavors (apartments).
...
// Extract the Booli detail-page path from a card's residence_url, accepting BOTH
// Booli flavors ("/bostad/<residenceId>" or "/annons/<booliId>"). Returns the path
// (e.g. "/annons/123") or null. Pass the result straight to fetchBooliDetail.
function extractDetailUrl(card) {
  if (!card || !card.residence_url) return null;
  const m = String(card.residence_url).match(/\/(?:bostad|annons)\/\d+/);
  return m ? m[0] : null;
}
  ...
  while (!reached() && page <= maxPages) {
    ...
    for (const c of cards) {
      ...
      if (shouldFetchDetail(c, seg, detailScope, maxSoldDate)) {
        const detailUrl = extractDetailUrl(c); // both /bostad/ and /annons/ flavors
        if (detailUrl) {
          let detail = null;
          try {
            detail = await fetchBooliDetail(detailUrl, { logger: log });
            detailFetches++;
          } catch (e) {
            if (e instanceof CeilingError) {
            ...
            log('WARN', `fetchBooliDetail ${detailUrl} error: ${e.message}`);
            detailErrors++;
          }
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // --- extractDetailUrl (both Booli flavors — the /annons fee-fetch fix) ---
  check('extractDetailUrl: /bostad/<id> → path', () => {
    assert.strictEqual(extractDetailUrl({ residence_url: '/bostad/12345' }), '/bostad/12345');
  });
  check('extractDetailUrl: /annons/<id> → path (was dropped by extractResidenceId)', () => {
    assert.strictEqual(extractDetailUrl({ residence_url: '/annons/678' }), '/annons/678');
    assert.strictEqual(extractResidenceId({ residence_url: '/annons/678' }), null); // contrast
  });
  check('extractDetailUrl: null/absent url → null', () => {
    assert.strictEqual(extractDetailUrl({ residence_url: null }), null);
    assert.strictEqual(extractDetailUrl({}), null);
  });
