...
// ---------------------------------------------------------------
async function postInfoMessage(channel, text) {
  if (!token()) return null;
  const json = await slackApiPost('chat.postMessage', { channel, text });
  if (!json) return null;
  return { ok: json.ok, ts: json.ts, channel: json.channel };
...
module.exports = { postReviewMessage, postDigestMessage, postInfoMessage, getReactions, dhashSummary, visionSummary, reviewLabel };
