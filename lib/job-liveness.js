...
//   * paused:'reason' — TEMPORARILY unscheduled (keeps its cron so it can be
//                       restored verbatim). Not supposed to run, so a missed
//                       fire is not a fault. Contrast `deprecated`, which is
//                       permanent.
function selectJobs(jobs) {
  return Object.entries(jobs)
    .filter(([, rec]) => rec.cron && !rec.shell && !rec.external && !rec.deprecated && !rec.paused)
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  check('a paused job is excluded from liveness entirely', () => {
    const jobs = {
      live:   { cron: '0 1 * * *', tier: 1, command: 'node live.js' },
      halted: { cron: '0 14 */2 * *', tier: 1, command: 'node halted.js',
                paused: 'Booli App Router outage' },
    };
    const names = selectJobs(jobs).map(j => j.job);
    assert.deepStrictEqual(names, ['live'],
      'a paused job is not scheduled, so reporting it as a missed run is a false alarm');
  });
