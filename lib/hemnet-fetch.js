...
//
// Pure CommonJS, native fetch (Node 20+). No new dependencies.
// Caller passes optional `logger(level, msg)` and optional `fetchImpl`
// (for testability). The lib stays silent unless a logger is supplied.
//
...
async function getWithRetry(targetUrl, opts = {}) {
  ...
  if (!fetchImpl) {
    throw new Error('hemnet-fetch: global fetch is not available (need Node 20+)');
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      const res = await fetchImpl(targetUrl, { headers: DEFAULT_HEADERS });
      ...
      // Other non-2xx, non-404, non-retryable: throw with body for debugging
      const body = await res.text().catch(() => '');
      throw new Error(
        `hemnet-fetch: ${targetUrl} returned ${res.status}: ${body.slice(0, 200)}`,
      );
    } catch (err) {
      // Non-retryable errors thrown above are caught here too — we re-throw
      // immediately if it's not a transient network error. Heuristic: if
      // the message starts with our prefix, surface immediately.
      if (err && typeof err.message === 'string' && err.message.startsWith('hemnet-fetch: ') && !err.message.includes('returned 5') && !err.message.includes('returned 429')) {
        throw err;
      }
      lastErr = err;
