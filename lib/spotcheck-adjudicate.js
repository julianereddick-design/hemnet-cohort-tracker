...
//   3. provisional==='suspect' && sharedPhoto===false && !priceAgrees
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // --- Price guard (D-03): price-agreeing suspect + sharedPhoto=false → UNCERTAIN, NOT CONFIRMED_MISMATCH ---
  check('suspect + sharedPhoto=false + price AGREES → UNCERTAIN (price guard, D-03)', () => {
    const r = rec({
      provisional: 'suspect',
      deltas: { price_pct_diff: 0.01 }, // price agrees — guard blocks CONFIRMED_MISMATCH
    });
    const result = adjudicatePair(r, { visionResult: { sharedPhoto: false } });
    assert.strictEqual(result.verdict, 'UNCERTAIN');
  });
