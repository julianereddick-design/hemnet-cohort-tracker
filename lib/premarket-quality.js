...
function tally(listings) {
  ...
  // Booli's searchForSaleV2 migration (2026-09) removed the AVM from search
  // cards entirely — a card now states a price or says "Pris ej angivet", and
  // no valuation is published alongside. A card whose source cannot show an AVM
  // sets avmObservable:false. If NO card could show one, pct_avm_shown is null
  // ("not measurable"), never 0.0%, which would read as "Booli showed none".
  // V1 cards leave the field undefined, so they count as observable as before.
  let avmObservable = 0;
  ...
  for (const c of listings) {
    ...
    if (INT(c)) interior++;
    if (P(c)) price++;
    if (c.avmObservable !== false) avmObservable++;
  ...
  return {
    ...
    pct_price: pct(price),
    pct_avm_shown: avmObservable > 0 ? pct(avm) : null,
