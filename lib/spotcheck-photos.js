...
// A download that never settles is worse than one that fails. If this promise hangs,
// the awaiting worker in spotcheck-photos.js never returns, Promise.all never resolves,
// and once the dead socket leaves the event loop node EXITS 0 with main() still pending
// — skipping the artifact write-back entirely and producing no error at all. That is the
// silent failure that broke the spot-check gate for 2026-W30/W31/W32 (303 of 305 pairs
// logged, then a 34ms silent exit). Root cause: `res.pipe(f)` neither finishes nor errors
// the destination when the SOURCE is destroyed mid-body, so the f.on('finish') /
// f.on('error') resolve paths never fire and a premature close emits nothing on `req`.
//
// Two guards, deliberately belt-and-braces because the cost of a hang is a whole silent
// weekly run: settle on response-stream errors too, and a hard deadline that can catch any
// future never-settle path regardless of which stream swallowed the event.
const DOWNLOAD_DEADLINE_MS = 60000;

function downloadImage(url, destPath, _redirects = 0) {
  return new Promise((resolveRaw) => {
    // settle-once: several handlers below can fire for the same failure.
    let done = false;
    let deadline = null;
    const resolve = (v) => {
      if (done) return;
    ...
    // NOT unref'd, deliberately. An unref'd timer does not hold the event loop open, so
    // if the dead socket were the last handle node would exit 0 before this ever fired —
    // reintroducing the exact silent exit this guard exists to prevent.
    deadline = setTimeout(
      () => resolve({ ok: false, error: `download-deadline-${DOWNLOAD_DEADLINE_MS}ms` }),
    ...
    const req = https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
      ...
      // The source must be handled too. A premature close mid-body destroys `res`
      // without emitting anything on `req` or on the destination, so these two
      // listeners are the only thing standing between a truncated image and a
      // permanently pending promise. 'aborted' is legacy but harmless to keep.
      res.on('error', (e) => { f.destroy(); resolve({ ok: false, error: `res:${e.message}` }); });
      res.on('aborted', () => { f.destroy(); resolve({ ok: false, error: 'res:aborted' }); });
      res.pipe(f);
      f.on('finish', () => f.close(() => {
        let bytes = 0;
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // --- downloadImage must ALWAYS settle (2026-W30/W31/W32 gate outage) ---
  // A never-settling download leaves its worker awaiting forever; once the dead socket
  // leaves the event loop, node exits 0 with main() pending and the artifact write-back
  // never runs — a whole weekly spot-check lost with no error. These lock the guards in.
  const srcAfterFix = fs.readFileSync(__filename, 'utf8');
  check('downloadImage: response-stream errors are handled (source destroyed mid-body)', () => {
    assert.ok(/res\.on\('error'/.test(srcAfterFix),
      "no res.on('error') — a premature close mid-body would never settle the promise");
    assert.ok(/res\.on\('aborted'/.test(srcAfterFix), "no res.on('aborted') handler");
  });
  check('downloadImage: has a hard deadline, and it is NOT unref\'d', () => {
    assert.ok(/DOWNLOAD_DEADLINE_MS/.test(srcAfterFix), 'no download deadline constant');
    assert.ok(!/deadline\.unref\(\)/.test(srcAfterFix),
      "the deadline is unref'd — it would not hold the loop open, so node would still exit 0 silently");
  });

  // Behavioural, offline: a refused connection must settle rather than hang.
  const asyncChecks = [];
  asyncChecks.push((async () => {
    const os2 = require('os');
    const path2 = require('path');
    const dest = path2.join(os2.tmpdir(), `dlimg-smoke-${process.pid}.jpg`);
    const raced = await Promise.race([
      downloadImage('https://127.0.0.1:1/nope.jpg', dest),
      new Promise((r) => setTimeout(() => r({ __hung: true }), 5000)),
    ]);
    check('downloadImage: connection refused settles (does not hang)', () => {
      assert.ok(!raced.__hung, 'promise did not settle within 5s');
