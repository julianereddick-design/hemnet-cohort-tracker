...
function makeSpendTally(opts = {}) {
  ...
  // WR-03 (mirrors Phase-15 WR-05): reject a non-positive / non-integer ceiling at
  // construction. Otherwise `calls < $2` (or `liveCalls >= max`) is always false/NaN
  // and the FIRST reserveCall throws a misleading "ceiling reached: NaN/NaN", silently
  // stalling the whole pipeline. Fail loud and early with the offending source instead.
  if (!Number.isInteger(max) || max <= 0) {
    const src = opts.max != null
      ? `max option (${JSON.stringify(opts.max)})`
      : `MAX_OXY_CALLS env (${JSON.stringify(process.env.MAX_OXY_CALLS)})`;
    throw new Error(`sold-spend: invalid Oxylabs ceiling from ${src} — expected a positive integer, got ${max}`);
  }
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  (async () => {
    ...
    // 7. WR-03: makeSpendTally rejects a non-positive / non-integer ceiling.
    check('makeSpendTally: invalid max (0, -1, NaN) throws at construction', () => {
      for (const bad of [0, -1, NaN, 3.5]) {
        assert.throws(
          () => makeSpendTally({ spendFile: path.join(os.tmpdir(), 'sold-spend-smoke-bad.json'), max: bad }),
          /invalid Oxylabs ceiling/,
          `expected throw for max=${bad}`,
        );
      }
      // Valid positive integer must still construct.
      assert.strictEqual(makeSpendTally({ spendFile: path.join(os.tmpdir(), 'sold-spend-smoke-ok.json'), max: 10 }).backend, 'file');
    });
