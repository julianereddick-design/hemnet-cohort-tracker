...
//     ⚠ CORRECTED 2026-08-19: that was verified by observing `crawled` held
//     recent values, not by checking which job WROTE them. On booli_listing it
//     was true (booli-targeted-refresh stamps crawled = NOW()). On
//     hemnet_listingv2 it was FALSE: hemnet-targeted-refresh updated rows
//     without ever touching `crawled`, which only looked fresh because the
//     Monday match job inserts rows with crawled = NOW(). hemnetRefreshRecent
//     therefore reported a healthy 5,167-listing run as a tier-1 failure, and
//     always would have. Fixed by making the refresh stamp `crawled`, with a
//     --smoke contract test in hemnet-targeted-refresh.js pinning it.
//     The lesson generalises: an assertion is only as good as the proof that
//     the job it watches writes the column it reads. Verify the WRITE, not the
...
const ASSERTIONS = {
  ...
  // The census writes one row per (platform, pool) — keyed that way in
  // age-census-store.js — so the four expected units of work are
  // booli/premarket, booli/forsale, hemnet/premarket, hemnet/forsale.
  // `pool` ALONE only ever holds {premarket, forsale}, so count(DISTINCT pool)
  // caps at 2 and a `>= 4` test could never pass: this fired a false tier-1
  // failure every day from 2026-09-01 while the job was returning
  // `status: success, persisted: 4`. Count the PAIR, matching the table's key.
  async ageCensusMonth(client, { lastFire }) {
    const r = await client.query(
      `SELECT count(DISTINCT (platform, pool))::int AS pools, count(*)::int AS n
         FROM age_census_run WHERE run_date >= $1::date`, [lastFire]);
    const { pools, n } = r.rows[0];
    return pools >= 4 ? ok(`${pools}/4 pools (${n} rows)`) : bad(`${pools}/4 platform×pool since the last expected run`);
  },
  ...
  // Migrated onto THIS droplet's cron 2026-08-18 (runs via runJob and does write
  // a cron_job_log row now), but a DB assertion stays the primary monitor here:
  // it is the only check that can tell a COMPLETE month from a degraded one.
  //
  // The window is 40 days, not 8. The scrape moved from weekly to MONTHLY on
  // 2026-08-17. An 8-day window against a monthly job is red for roughly three
  // weeks in every four — a standing false alarm, which is exactly the noise
  // this alerting design exists to escape. 40 days covers a 31-day month plus
  // slack while still catching a genuinely dead crawler within about a week of
  // the month it missed.
  //
  // Completeness is asserted against the NEWEST crawl day only, not "any
  // complete grid in the window". Under the old rule an older complete grid
  // could vouch for a newer, degraded one: on 2026-09-01 the 2026-08-17 grid
  // would satisfy "some complete grid exists in 40 days" and keep satisfying it
  // until 2026-09-26, so a September crawl that writes a partial grid and dies —
  // the exact shape of 2026-08-02 (42 cells) and 2026-08-09 (35 cells), both
  // still in the table — would read healthy for 25 days. The newest day is the
  // one that matters: if it is incomplete, the month is bad, full stop.
...
async function runAssertions(client, jobs, { now, rows = [] }) {
  ...
  for (const [job, rec] of Object.entries(jobs)) {
    ...
    // A paused job is deliberately unscheduled, so its data cannot arrive and
    // its assertion would fail every run. Skip it exactly as if it had no cron.
    if (rec.paused) continue;
    // An external job (one that runs somewhere outside this crontab and writes
    // no cron_job_log row here — no such job exists in the registry as of
    // 2026-08-18, now that ad-cost-crawler has been migrated onto this droplet
    // and lost its `external: true`) has no cron here, so the cron-timing skip
    // rules cannot apply. Its assertion is self-contained over a rolling window.
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // age_census_run is keyed (run_date, platform, pool) and `pool` holds only
  // {premarket, forsale}. Counting DISTINCT pool alone caps at 2, so any
  // threshold above 2 is unreachable — the 2026-09 false-positive. Generalise
  // the lesson: never count DISTINCT on one half of a composite key.
  check('no assertion counts DISTINCT on half of a composite key', () => {
    const fs = require('fs');
    const src = fs.readFileSync(__filename, 'utf8');
    const body = src.slice(src.indexOf('const ASSERTIONS'), src.indexOf('function shouldEvaluate'))
      .split('\n')
      ...
      .join('\n');
    assert.ok(!/count\(DISTINCT\s+pool\s*\)/i.test(body),
      'count(DISTINCT pool) caps at 2 — age_census_run is keyed (platform, pool); count the pair');
  });
  ...
  (async () => {
    ...
    await checkA('runAssertions skips a paused tier-1 job', async () => {
      const fakeClient = { query: async () => { throw new Error('a paused job must not be queried'); } };
      const res = await runAssertions(fakeClient, {
        'booli-targeted-refresh': {
          tier: 1, cron: '0 14 */2 * *', expectedDurationMin: 60,
          assert: 'booliRefreshRecent', paused: 'Booli App Router outage',
        },
      }, { now, rows: [] });
      assert.deepStrictEqual(res, [],
        'a paused job is not scheduled, so asserting its data arrived is a guaranteed false alarm');
    });
    ...
    // Regression for the 2026-09 false positive: a month in which the census
    // persisted all four platform×pool rows must read as HEALTHY. The old SQL
    // asked Postgres for count(DISTINCT pool), which returns 2 for exactly this
    // data, so the job was reported as failing every day while it was fine.
    await checkA('age census: 4 platform×pool rows persisted reads as healthy', async () => {
      const seen = [];
      const fakeClient = {
        query: async (sql) => {
          seen.push(sql);
          // Stand in for Postgres over the four real rows:
          //   (booli|hemnet) × (premarket|forsale)
          const rows = [
            { platform: 'booli',  pool: 'premarket' }, { platform: 'booli',  pool: 'forsale' },
            { platform: 'hemnet', pool: 'premarket' }, { platform: 'hemnet', pool: 'forsale' },
          ];
          const distinct = /count\(DISTINCT\s*\(\s*platform\s*,\s*pool\s*\)\s*\)/i.test(sql)
            ? new Set(rows.map(r => `${r.platform}/${r.pool}`)).size
            : new Set(rows.map(r => r.pool)).size;   // the old, broken phrasing
          return { rows: [{ pools: distinct, n: rows.length }] };
        },
      };
      const res = await runAssertions(fakeClient, {
        'age-census-monthly': {
          tier: 1, cron: '0 2 1 * *', expectedDurationMin: 240, assert: 'ageCensusMonth',
        },
        // 06:00 past the 02:00 fire clears the 240min duration budget, so the
        // assertion is actually evaluated rather than skipped as in-flight.
      }, { now: new Date('2026-09-01T08:00:00Z'), rows: [] });
      assert.strictEqual(res.length, 1, 'the assertion must be evaluated, not skipped');
      assert.strictEqual(res[0].ok, true,
        `a fully successful census must not raise an issue (detail: ${res[0].detail})`);
      assert.match(seen.join(' '), /platform/,
        'the assertion must count the (platform, pool) pair, not pool alone');
    });
