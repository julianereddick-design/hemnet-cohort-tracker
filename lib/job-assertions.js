...
const ASSERTIONS = {
  ...
  async cohortTrackViews(client, { lastFire }) {
    const r = await client.query(
      `SELECT (SELECT count(*)::int FROM cohort_daily_views WHERE date >= $1::date) AS n_views,
              (SELECT count(*)::int FROM cohort_pairs
                WHERE removed_at IS NULL
                  AND (dropped_booli_on IS NULL OR dropped_hemnet_on IS NULL)) AS n_active`, [lastFire]);
  ...
  async adCostWeek(client) {
    const r = await client.query(
      `SELECT count(*)::int AS n, max(crawled) AS latest FROM hemnet_adcostv2
        WHERE crawled >= NOW() - INTERVAL '8 days'`);
    const { n, latest } = r.rows[0];
    return n > 0 ? ok(`${n} rows, latest ${new Date(latest).toISOString().slice(0, 16)}Z`)
                 : bad('no hemnet_adcostv2 rows in 8 days — the other droplet may be down');
  },
};
