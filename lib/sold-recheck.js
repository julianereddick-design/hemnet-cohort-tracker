...
// Numeric columns that feed the MATCH COMPARISONS. Postgres `numeric` comes back from
// node-pg as a STRING (it preserves precision rather than risking a lossy float), but
// lib/spotcheck-evidence gates every comparison on `typeof n === 'number'` (finitePos).
// So a DB-loaded record silently produced price_pct_diff = area_pct_diff = null, the
// adjudicator was left with one signal where it needs two, and every re-check returned
// UNCERTAIN — after which runRecheck dropped the row from the queue permanently.
//
// The FIRST pass never hit this: it builds records from the live scrape, where these are
// real numbers. Only this DB-load path was affected — 816/816 re-checked rows in prod.
//
// booli_id / residence_id / agent_id / agency_id are bigint and DELIBERATELY excluded:
// they are identifiers, never compared numerically, and have been strings since day one.
// lat/long/plot_area/additional_area/operating_cost are excluded too — not comparison
// inputs. Keep this list to fields the matcher actually reasons over.
const RECORD_NUMERIC_FIELDS = ['sold_price', 'living_area', 'rooms', 'rent', 'floor'];

// coerceRecordNumerics — string→number for the comparison fields ONLY, and only when the
// string is genuinely numeric. null stays null (never 0); '' and non-numeric text are left
// untouched (never NaN) so a bad value fails the finitePos gate exactly as it does today.
function coerceRecordNumerics(row) {
  if (!row || typeof row !== 'object') return row;
  const out = { ...row };
  for (const f of RECORD_NUMERIC_FIELDS) {
    const v = out[f];
...
// Both paths go through coerceRecordNumerics so an injected stub cannot mask the defect.
async function loadBooliRecord(client, booliId, deps) {
  if (deps && deps.loadBooliRecord) return coerceRecordNumerics(await deps.loadBooliRecord(booliId));
  const r = await client.query(`SELECT * FROM booli_sold WHERE booli_id = $1`, [booliId]);
  return coerceRecordNumerics(r.rows[0] || null);
}
...
module.exports = {
  ...
  runRecheck, resolveMatchOne, loadBooliRecord, coerceRecordNumerics,
};
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  (async () => {
    ...
    await checkAsync('runRecheck: late match flips + clears', async () => {
      ...
      const clearCall = c.calls.find((q) => /UPDATE sold_match\s+SET recheck_until = NULL/.test(q.sql));
      assert.ok(clearCall, 'clearRecheck ran (nulls scheduling state) on the late-matched row');
      // first_unmatched_at MUST be PRESERVED — it is the late-resolved marker Phase-20 uses
      // to decompose first-scrape vs late-added matches.
      assert.ok(!/first_unmatched_at = NULL/.test(clearCall.sql),
        'clearRecheck must NOT null first_unmatched_at (preserve the late-resolved marker)');
    });
    ...
    await checkAsync('runRecheck: uncertain clears state, never advanced/settled', async () => {
      ...
      assert.ok(c.calls.some((q) => /UPDATE sold_match\s+SET recheck_until = NULL/.test(q.sql)),
    ...
    // ---------------------------------------------------------------------
    // 14-16. REGRESSION for the 2026-08-14 numeric-string defect.
    //
    // Postgres `numeric`/`bigint` come back from node-pg as STRINGS (precision is
    // preserved rather than silently truncated to a float). The comparison helpers in
    // lib/spotcheck-evidence gate on `typeof n === 'number'` (finitePos), so a record
    // loaded from the DB produced price_pct_diff = area_pct_diff = null on EVERY
    // re-check. With no price/area signal the adjudicator had only the fee — one signal,
    // where it requires two — so it returned UNCERTAIN, and runRecheck then dropped the
    // row from the queue for good. Measured in prod: 816/816 re-checked rows had a null
    // price delta, vs 3110/3491 first-pass rows carrying real values (the first pass
    // reads numbers straight off the scrape, so it was never affected).
    // ---------------------------------------------------------------------
    await checkAsync('14. loadBooliRecord coerces DB numeric STRINGS to numbers', async () => {
      const c = mockClient();
      c.rowsToReturn = [{
        booli_id: '6073395', sold_price: '3800000', living_area: '98',
        rooms: '4', rent: '2450', floor: '3', street_address: 'Testgatan 1',
      }];
      const rec = await loadBooliRecord(c, 6073395, {});
      assert.strictEqual(typeof rec.sold_price, 'number', 'sold_price must be a number');
      assert.strictEqual(rec.sold_price, 3800000, 'sold_price value preserved');
      assert.strictEqual(typeof rec.living_area, 'number', 'living_area must be a number');
      assert.strictEqual(typeof rec.rooms, 'number', 'rooms must be a number');
      assert.strictEqual(typeof rec.rent, 'number', 'rent must be a number (fee signal)');
      assert.strictEqual(typeof rec.floor, 'number', 'floor must be a number (unit signal)');
      // booli_id is an IDENTIFIER, deliberately left as-is: it has been a string for
      // months, is never compared numerically, and retyping it would touch upserts/joins.
      assert.strictEqual(typeof rec.booli_id, 'string', 'booli_id deliberately NOT coerced');
    });

    await checkAsync('15. a coerced record yields NON-NULL deltas (the defect consequence)', async () => {
      const { computeDeltas } = require('./spotcheck-evidence');
      const c = mockClient();
      c.rowsToReturn = [{
        sold_price: '3800000', living_area: '98',
        object_type: 'Lägenhet', street_address: 'Testgatan 1',
      }];
      const rec = await loadBooliRecord(c, 1, {});
      const d = computeDeltas(
        { price: rec.sold_price, living_area: rec.living_area, object_type: rec.object_type, street_address: rec.street_address, postcode: null },
        { asking_price: 3800000, living_area: 98, housing_form: 'Lägenhet', street_address: 'Testgatan 1', post_code: null },
      );
      assert.notStrictEqual(d.price_pct_diff, null, 'price delta must not be null — this is the whole bug');
      assert.notStrictEqual(d.area_pct_diff, null, 'area delta must not be null — this is the whole bug');
      assert.strictEqual(d.price_pct_diff, 0, 'identical prices → 0% difference');
      assert.strictEqual(d.area_pct_diff, 0, 'identical areas → 0% difference');
    });

    await checkAsync('16. coercion is safe: nulls, blanks and non-numeric text untouched', async () => {
      const c = mockClient();
      c.rowsToReturn = [{
        sold_price: null, living_area: '', rooms: 'abc',
        street_address: 'Testgatan 1', object_type: 'Lägenhet', rent: 2450,
      }];
      const rec = await loadBooliRecord(c, 1, {});
      assert.strictEqual(rec.sold_price, null, 'null stays null (never becomes 0)');
      assert.strictEqual(rec.living_area, '', 'empty string left alone (never becomes 0)');
      assert.strictEqual(rec.rooms, 'abc', 'non-numeric text left alone (never becomes NaN)');
      assert.strictEqual(rec.street_address, 'Testgatan 1', 'text fields untouched');
      assert.strictEqual(rec.rent, 2450, 'values already numeric pass through unchanged');
    });
