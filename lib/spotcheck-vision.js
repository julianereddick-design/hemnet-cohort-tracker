...
const { filterDiscriminating } = require('./spotcheck-dhash');
...
async function adjudicateWithVision(pair, opts = {}) {
  ...
  // D-05/D-10: exclude non-discriminating images (floorplans/renders/maps) BEFORE
  // building the payload — a shared floorplan fools vision exactly like it fools
  // dHash (two units in one development share the identical floorplan image).
  // Then slice to budget cap.
  const hemnetSlice = filterDiscriminating(hemnetGallery).slice(0, maxImagesPerSide);
  const booliSlice  = filterDiscriminating(booliGallery).slice(0, maxImagesPerSide);
...
CRITICAL: a shared floorplan drawing, developer render, or marketing diagram is NOT evidence of the same property — different units in one building or new-build development reuse identical floorplans and renders. If the ONLY shared feature is a floorplan/render/diagram, answer null, not true. In new-builds, identically staged units also look alike — require a genuinely distinctive shared detail (view, fixed fittings, wear marks), or answer null.
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  (async () => {
    ...
    // CR-01: without this catch an uncaught rejection (e.g. in teardown) would let
    // the process exit 0 — a false-green smoke signal. Fail loudly instead.
