...
const JOBS = {
  ...
  // Cadence changed weekly -> MONTHLY on 2026-08-17 (celery-beat `0 2 1 * *` UTC,
  // deliberately re-anchored to UTC so it cannot drift against the UTC report cron
  // when Sydney DST flips). The assertion window moved 8d -> 40d with it; leaving it
  // at 8 days would have gone red about three weeks in every four.
  'ad-cost-crawler': {
    tier: 1, external: true, frequency: 'monthly', label: 'Monthly (1st 02:00, other droplet)',
    assert: 'adCostMonth',
  },
