...
const JOBS = {
  ...
  'booli-targeted-refresh': {
    // PAUSED 2026-09-22. Booli moved to the Next.js App Router some time between
    // 05:55 and 08:30 UTC on 2026-09-21, so `__NEXT_DATA__` no longer exists and
    // every fetch fails in the transport layer. The 2026-09-21 run burned 6,134
    // Oxylabs calls for 0 parsed listings and would repeat that every 2 days
    // (~8% of the monthly cap per week). `cron` is retained so the job can be
    // restored by deleting this one field once the extractor is fixed.
    tier: 1, frequency: 'every2days', label: 'Every 2 days (14:00)',
    ...
    paused: 'Booli App Router migration 2026-09-21 — every fetch fails, 6,134 calls/run wasted',
  },
