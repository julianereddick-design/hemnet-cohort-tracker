...
// Defensive ceiling on the per-segment /slutpriser walk.
//
// Until Booli's searchSoldV2 migration (2026-09) the walk had TWO terminators:
// an empty page, and `page >= meta.pages`. V2 dropped `pages` from the node, so
// the page-count stop silently stopped firing and an empty page became the only
// way out. A segment is one municipality x family over a ~14-day window, which
// is normally 1-3 pages, so this is far above any legitimate walk — it exists so
// a feed that never returns an empty page cannot spin. The batch-wide Oxylabs
// CeilingError is the outer net; this keeps one bad segment from eating it.
const MAX_SOLD_PAGES = 60;
  ...
  for (const muni of panel.munis) {
    ...
    for (const family of FAMILIES) {
      ...
      while (true) {
        ...
        if (page >= MAX_SOLD_PAGES) {
          log('WARN',
            `sampleNational ${segKey}: hit MAX_SOLD_PAGES=${MAX_SOLD_PAGES} without an empty page — ` +
            'feed may not terminate; walk stopped');
          break;
        }
        page++;
      }
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  (async () => {
    ...
    // searchSoldV2 removed `pages`, so an empty page became the walk's ONLY
    // terminator. A feed that never returns one must still stop.
    await checkAsync('a never-empty feed stops at MAX_SOLD_PAGES', async () => {
      let maxPageSeen = 0;
      const warns = [];
      const endless = async (segKey, seg, fopts) => {
        maxPageSeen = Math.max(maxPageSeen, fopts.page);
        // V2 shape: a real numeric totalCount, and NO pages field at all.
        return {
          cards: [{ booli_id: `${segKey}-${fopts.page}`, is_title_transfer: false }],
          meta: { totalCount: 460607, pages: null },
        };
      };
      await sampleNational({
        log: (lvl, msg) => { if (lvl === 'WARN') warns.push(msg); },
        deps: {
          panel: { target_sample_size: 10, lookback_days: 14,
            munis: [{ name: 'Stockholm', pop: 980, booli_area_id: 1, hemnet_location_id: 18031 }] },
          fetchBooliSoldPage: endless,
          knownBooliIds: async () => new Set(),
          now: '2026-06-18',
        },
      });
      assert.strictEqual(maxPageSeen, MAX_SOLD_PAGES,
        `walk must stop at the ceiling, reached page ${maxPageSeen}`);
      assert.ok(warns.some(w => /MAX_SOLD_PAGES/.test(w)), 'hitting the ceiling must warn');
    });
