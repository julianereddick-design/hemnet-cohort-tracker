...
function applyOutcome(state, { condition, alerted, now }) {
  ...
  return Object.assign({}, base, {
    ...
    // Occurrences are counted SEPARATELY from alerts. A tier-2 condition never
    // alerts, so alert_count stays 0 forever and cannot carry the number the
    // digest has to show ("continuing since X, 5 consecutive" — §7 acceptance).
    seen_count: (base.seen_count || 0) + 1,
  });
}
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // §7 acceptance: a tier-2 warning repeated 5 runs must produce a DIGEST LINE
  // reading "continuing since X, 5 consecutive". Tier 2 never alerts, so
  // alert_count stays 0 and cannot carry that number — the occurrence count has
  // to be tracked separately from the alert count.
  check('seen_count counts occurrences even when nothing is ever alerted', () => {
    let state = null;
    const c = { key: 'stale-reviews', severity: 'warning', message: '17 unanswered' };
    for (let i = 0; i < 5; i++) {
      state = applyOutcome(state, { condition: c, alerted: false, now: hours(i * 24) });
    }
    assert.strictEqual(state.seen_count, 5, 'five runs, five occurrences');
    assert.strictEqual(state.alert_count, 0, 'and not one alert');
    assert.strictEqual(state.first_seen_at.getTime(), t0.getTime(),
      '"continuing since" needs the ORIGINAL sighting, not the latest');
  });

  check('a flap does not restart the occurrence count', () => {
    const c = { key: 'null-views', severity: 'warning', message: 'x' };
    let state = applyOutcome(null, { condition: c, alerted: false, now: t0 });
    state = applyOutcome(state, { condition: null, alerted: false, now: hours(2) });
    state = applyOutcome(state, { condition: c, alerted: false, now: hours(4) });
    assert.strictEqual(state.seen_count, 2, 'the incident continued, so the count continued');
  });
