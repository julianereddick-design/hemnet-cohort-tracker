...
// routes to human review), clear scheduling state so the row leaves the drain queue.
// Verdict itself is set by the matchOne persist; this only nulls the state. Guarded
// `AND verdict <> 'booli_only'` (WR-03) so a still-active booli_only row can never be
// accidentally dropped from the queue by a stray caller — clear only fires once the
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  (async () => {
    ...
    // 13. clearRecheck nulls all three scheduling columns (late-match cleanup).
    await checkAsync('clearRecheck: nulls all three scheduling columns', async () => {
      const c = mockClient();
      await clearRecheck(c, 3);
      assert.ok(c.calls[0].sql.includes('first_unmatched_at = NULL'), 'must null first_unmatched_at');
