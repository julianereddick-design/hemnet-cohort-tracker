...
async function fetchBooliSearch(areaId, opts = {}) {
  ...
  const apolloState = booliStateFromHtml(res.html, 'search', `areaId=${areaId} page=${page}`);
...
// booliStateFromHtml(html, kind, label) — hand back the state object the
// parsers expect, whichever router Booli is serving.
//
// Booli moved Pages Router -> App Router on 2026-09-21 and the two page types
// did NOT move the same way:
//   search pages kept the normalised Apollo cache, relocated from
//     props.pageProps.__APOLLO_STATE__ to initialApolloState in the flight payload;
//   detail pages dropped the cache entirely and are now a denormalised tree,
//     so entities are rebuilt into the flat `Typename:id` map parseBooliListing
//     addresses by key.
// Both Pages Router branches are retained deliberately: Hemnet has not migrated,
// and keeping them means a Booli rollback needs no code change.
function booliStateFromHtml(html, kind, label) {
  if (typeof html === 'string' && html.includes('self.__next_f')) {
    return kind === 'detail' ? extractFlightEntities(html) : extractFlightData(html);
  }
  const data = extractNextData(html);
  const st = data && data.props && data.props.pageProps && data.props.pageProps.__APOLLO_STATE__;
  if (!st) {
    throw new Error(`booli-fetch: no __APOLLO_STATE__ in ${kind} for ${label}`);
  }
  return st;
}
...
async function fetchBooliDetail(listingUrl, opts = {}) {
  ...
  const apolloState = booliStateFromHtml(res.html, 'detail', absoluteUrl);
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // Independent coverage of the saleType path. On the live NORMAL node cards
  // carry saleType with primaryStatus null, so saleType is the signal that would
  // actually catch a new-build appearing there — it must stand on its own.
  check('searchV2: saleType alone is enough to flag a new-build', () => {
    const apollo = {
      ROOT_QUERY: { 'searchForSaleV2({"page":1})': { [ITEMS]: [{ __ref: 'ListableProperty:n' }] } },
      'ListableProperty:n': v2Card(9, 'Inkommet idag', {
        saleType: 'newConstruction', primaryStatus: null,
      }),
    };
  ...
  check('booliStateFromHtml reads App Router search pages', () => {
    const inner = '1:[{"initialApolloState":{"ROOT_QUERY":{"__typename":"Query"}}}]';
    const html = `<script>self.__next_f.push([1,${JSON.stringify(inner)}])</script>`;
    const st = booliStateFromHtml(html, 'search', 'test');
    assert.ok(st.ROOT_QUERY, 'App Router search must yield the Apollo cache');
  });

  check('booliStateFromHtml still reads Pages Router search pages', () => {
    ...
    const st = booliStateFromHtml(html, 'search', 'test');
    assert.strictEqual(st.ROOT_QUERY.a, 1, 'the pre-2026-09-21 shape must keep working');
  });

  check('booliStateFromHtml rebuilds entities for App Router detail pages', () => {
    const row = { __typename: 'Listing', id: '99', url: '/annons/99' };
    const inner = '1:' + JSON.stringify([row]);
    const html = `<script>self.__next_f.push([1,${JSON.stringify(inner)}])</script>`;
    const st = booliStateFromHtml(html, 'detail', 'test');
    assert.ok(st['Listing:99'], 'detail pages are denormalised — entities must be rebuilt');
  });
