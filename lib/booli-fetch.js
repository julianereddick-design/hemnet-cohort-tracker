...
function parseBooliSearchCards(apolloState, opts = {}) {
  ...
  for (const ref of refs) {
    ...
    cards.push({
      ...
      objectType: typeof listing.objectType === 'string' ? listing.objectType : null,
      // V2 dropped the per-card isNewConstruction boolean. Within the node we
      // select, new-construction arrives as 'project:' tiles, which are filtered
      // out above — so this is false by construction here. premarket-flow-measure
      // counts new-builds off this field and must be revisited separately.
