...
// Bands that a given SOURCE cannot resolve, and therefore must report as null
// rather than 0. Those mean opposite things: 0 says "we looked and found none",
// null says "this source cannot tell us".
//
// Booli's searchForSaleV2 cards state age as prose, quantised to whole days for
// the first month, then 'N månader', then 'N år'. Verified live 2026-09-21 on
// pre-market pages 300/500/600/700/830 — the full vocabulary maps, but the
// GRANULARITY does not: '1 år på Booli' is the only value between 365 and 730
// days, and it covers every real age from 12 to 23 months. Edge 548 (18mo) sits
// inside that single value, so nothing can ever land in [548, 730):
//
//   le1m  m1_3  m3_6  m6_12  m12_18        m18_24          gt24
//   ok    ok    ok    ok     absorbs 12-24 UNREACHABLE     ok
//
// So m18_24 would read a permanent, entirely credible 0 while m12_18 quietly
// carried both. Report it as null, and read m12_18 as 12-24mo for Booli.
// Hemnet is unaffected — its cards carry real publishedAt timestamps.
// Merge SOURCE band -> TARGET band, then report the source as null. Merging
// rather than dropping keeps the census's `bands + undated === nTotal`
// reconciliation intact: the counts are not lost, they are attributed to the
// band that genuinely holds them. On live Booli data the source band is always
// empty anyway (nothing can map into it), so this is a labelling correction
// first and a safety net second.
const BOOLI_BAND_MERGE = { m18_24: 'm12_18' };

function applyBandMerge(o, merge) {
  for (const [src, dst] of Object.entries(merge || {})) {
    if (!(src in o) || !(dst in o)) continue;
    o[dst] = (o[dst] || 0) + (o[src] || 0);
    o[src] = null;
  }
  return o;
}

function bucketsToObject(buckets, undated, opts = {}) {
  const o = {};
  BAND_KEYS.forEach((key, k) => { o[key] = buckets[k] || 0; });
  o.undated = undated || 0;
  return applyBandMerge(o, opts.mergeBands);
}
...
function secondhandToObject(buckets, newbuild, undated, opts = {}) {
  const o = {};
  BAND_KEYS.forEach((key, k) => {
    o[key] = Math.max(0, (buckets[k] || 0) - (newbuild[k] || 0));
  });
  o.undated = undated || 0;
  return applyBandMerge(o, opts.mergeBands);
}
...
module.exports = {
  ...
  bucketsToObject, secondhandToObject, BOOLI_BAND_MERGE, applyBandMerge,
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  // Booli cannot resolve [548,730): '1 år på Booli' is the only value between
  // 365 and 730 days and it spans 12-23 months. 0 would read as "none found".
  check('an unresolvable band merges into its neighbour and reports null', () => {
    const buckets = [10, 20, 30, 40, 50, 60, 70];
    const newbuild = [1, 2, 3, 4, 5, 6, 7];
    const opts = { mergeBands: BOOLI_BAND_MERGE };
    const all = bucketsToObject(buckets, 3, opts);
    const sec = secondhandToObject(buckets, newbuild, 3, opts);
    assert.strictEqual(all.m18_24, null, 'Booli cannot resolve [548,730)');
    assert.notStrictEqual(all.m18_24, 0, '0 would claim we looked and found none');
    assert.strictEqual(all.m12_18, 110, 'its count moves to m12_18, which is 12-24mo for Booli');
    assert.strictEqual(sec.m18_24, null);
    assert.strictEqual(sec.m12_18, (50 - 5) + (60 - 6), 'second-hand merges the same way');
    assert.strictEqual(all.gt24, 70, 'other bands untouched');
  });
  // The census asserts bands + undated === nTotal. Dropping a band breaks that;
  // merging must not.
  check('merging preserves the band total (the census reconciliation)', () => {
    const buckets = [10, 20, 30, 40, 50, 60, 70];
    const plain = bucketsToObject(buckets, 3);
    const merged = bucketsToObject(buckets, 3, { mergeBands: BOOLI_BAND_MERGE });
    const sum = o => BAND_KEYS.reduce((a, k) => a + (o[k] || 0), 0);
    assert.strictEqual(sum(plain), 280);
    assert.strictEqual(sum(merged), 280, 'no count may be lost by the merge');
  });
  check('with no merge configured nothing changes (Hemnet path)', () => {
    const all = bucketsToObject([1,2,3,4,5,6,7], 0);
    assert.strictEqual(all.m18_24, 6, 'Hemnet has real timestamps — all bands resolve');
  });
