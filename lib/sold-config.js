...
// Both adjustable via env with NO code edit.
// Window default 182d (~6 months): slutpris can lag months, so we keep re-checking
// a booli_only sale for a Hemnet appearance for that long before settling it terminal.
// The batch fires fortnightly, so a due row is re-checked once per run (~13 passes
// over the window); the 7d interval just guarantees it is always due each run.
// NOTE: recheck_until is frozen at enroll time, so a change here applies only to rows
// enrolled afterward — extending in-flight rows requires a one-off backfill UPDATE.
const RECHECK_WINDOW_DAYS = posIntEnv('RECHECK_WINDOW_DAYS', 182);
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  check('RECHECK_WINDOW_DAYS default is 182 (~6 months)', () => {
    assert.strictEqual(RECHECK_WINDOW_DAYS, 182);
  });
