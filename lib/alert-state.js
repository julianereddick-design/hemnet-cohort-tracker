...
// Added after alert_state shipped, so an ALTER is needed on an existing table.
// seen_count counts OCCURRENCES, which alert_count cannot: a tier-2 condition
// never alerts, so its alert_count stays 0 no matter how long it persists.
const DDL_ALTER = `ALTER TABLE alert_state ADD COLUMN IF NOT EXISTS seen_count INTEGER NOT NULL DEFAULT 0`;
  ...
  try {
    const r = await client.query(
      `SELECT condition_key, first_seen_at, last_seen_at, first_alerted_at,
              last_alerted_at, alert_count, seen_count, consecutive_clear
         FROM alert_state
...
// loadOpen -> EVERY open condition for a script. The N=2 flap debounce only
// works if a CLEAN run still ticks the open incidents forward, and a clean run
// does not know which keys are open — so it cannot ask for them by key.
// Degrades to [] rather than throwing: an absent table must not fail the job.
async function loadOpen(client, scope, scriptName) {
  try {
    const r = await client.query(
      `SELECT condition_key, first_seen_at, last_seen_at, first_alerted_at,
              last_alerted_at, alert_count, seen_count, consecutive_clear
         FROM alert_state
        WHERE scope = $1 AND script_name = $2`,
      [scope, scriptName],
    );
    return r.rows;
  } catch (err) {
    ...
    await client.query(
      `INSERT INTO alert_state
         (scope, script_name, condition_key, first_seen_at, last_seen_at,
          first_alerted_at, last_alerted_at, alert_count, seen_count, consecutive_clear)
       VALUES ($1,$2,$3, COALESCE($4, NOW()), COALESCE($5, NOW()), $6, $7, $8, $9, $10)
      ...
      [scope, scriptName, state.condition_key,
       ...
       state.alert_count || 0, state.seen_count || 0, state.consecutive_clear || 0],
    );
  } catch (err) {
...
// listOpen(scope) -> every open condition across ALL scripts, for the digest's
// "continuing since" section.
async function listOpen(client, scope) {
  try {
    const r = await client.query(
      `SELECT script_name, condition_key, first_seen_at, last_seen_at,
              alert_count, seen_count, consecutive_clear
         FROM alert_state WHERE scope = $1 ORDER BY first_seen_at`, [scope]);
    return r.rows;
  } catch (err) {
    if (isMissingTable(err)) return [];
    throw err;
...
module.exports = { DDL, DDL_ALTER, ensureTable, loadState, loadOpen, listOpen, saveState, clearState, isMissingTable, MISSING_TABLE_CODE };
...
if (require.main === module && process.argv.includes('--smoke')) {
  ...
  const { DDL, loadState, loadOpen, saveState, clearState, MISSING_TABLE_CODE } = module.exports;
  let pass = 0, fail = 0;
  ...
  (async () => {
    ...
    // The N=2 flap debounce only works if a CLEAN run still ticks the open
    // incidents forward, and a clean run does not know which keys are open.
    await checkA('loadOpen returns every open condition for a script', async () => {
      const c = fakeClient([{ rows: [{ condition_key: 'a' }, { condition_key: 'b' }] }]);
      const rows = await loadOpen(c, 'run', 'cohort-track');
      assert.strictEqual(rows.length, 2);
      assert.deepStrictEqual(c.calls[0].params, ['run', 'cohort-track']);
    });

    await checkA('loadOpen returns an empty array, never null, when nothing is open', async () => {
      assert.deepStrictEqual(await loadOpen(fakeClient([{ rows: [] }]), 'run', 'x'), []);
    });

    await checkA('loadOpen on a missing table degrades to empty, not to a crash', async () => {
      assert.deepStrictEqual(await loadOpen(fakeClient([missingTable()]), 'run', 'x'), []);
    });
