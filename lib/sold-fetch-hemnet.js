...
//   - If remainingCallsAsync() <= 40 before fetching a page, stop draining and
//     return partial with complete=false, stopReason='ceiling-floor'
//     (async DB tally — the sync remainingCalls() is inert once a Phase-17 driver
//      calls setSpendClient, so this guard MUST read the authoritative DB count)
...
async function searchSoldPaged(booli, seg, windowDays, maxPages, opts) {
  ...
  for (; page <= maxPages; page++) {
    // Drain guard: stop before hitting the hard ceiling. Async DB tally — the sync
    // remainingCalls() is pinned once setSpendClient switches to the DB backend.
    if ((await remainingCallsAsync()) <= 40) {
