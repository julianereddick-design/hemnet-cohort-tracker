  ...
  (async () => {
    ...
    await checkAsync('no reporter carries its own sendSlack or reads SLACK_WEBHOOK_URL directly', async () => {
      ...
      const migrated = [
        ...
        'cron-health-slack.js', 'sold-match-report.js',
      ];
    ...
    await checkAsync('sold-match-report posts via postMessage, not the dry-run-blind postInfoMessage', async () => {
      const fs = require('fs');
      const path = require('path');
      const src = fs.readFileSync(path.join(__dirname, '..', 'sold-match-report.js'), 'utf8');
      // postInfoMessage (lib/spotcheck-slack-bot.js) never consults isDryRun() — only
      // SLACK_BOT_TOKEN presence. A revert back to it would make --dry-run POST for real
      // while looking safe. Locks in the Task 7 fix.
      assert.ok(!/postInfoMessage\s*\(/.test(src), 'sold-match-report.js calls postInfoMessage — --dry-run would not stop a real post');
      assert.ok(/postMessage\s*\(\s*['"]sold-match-report['"]/.test(src), 'sold-match-report.js does not post via postMessage');
    });
    ...
    // postMessage signals a failed delivery by RETURNING {ok:false}, never by throwing.
    // A caller that only try/catches it will log "sent" for a report that was lost and
    // exit 0. None of these six run under cron-wrapper.runJob, so the exit code is the
    // only failure signal they have.
    await checkAsync('every reporter branches on result.ok and fails non-zero', async () => {
      const fs = require('fs');
      const path = require('path');
      const root = path.join(__dirname, '..');
      const reporters = [
      ...
      for (const f of reporters) {
        const src = fs.readFileSync(path.join(root, f), 'utf8');
        assert.ok(/=\s*await\s+postMessage\(/.test(src),
          `${f} discards postMessage's result — a failed delivery cannot be detected`);
        assert.ok(/result\.ok/.test(src), `${f} never checks result.ok`);
        assert.ok(/process\.exitCode\s*=\s*1/.test(src),
          `${f} does not exit non-zero on a failed post — the failure would be invisible`);
      }
    });

    // cron-wrapper's alert must be guarded: postAlert can REJECT (new URL on a malformed
    // webhook), and an unhandled rejection there skips client.end() and process.exit()
    // for every job that routes through runJob.
    check('cron-wrapper guards its postAlert call', () => {
      const fs = require('fs');
      const path = require('path');
      const src = fs.readFileSync(path.join(__dirname, '..', 'cron-wrapper.js'), 'utf8');
      const m = src.match(/try\s*\{[^}]*await\s+postAlert\([\s\S]{0,400}?\}\s*catch/);
      assert.ok(m, 'await postAlert() is not inside a try/catch in cron-wrapper.js');
    });
