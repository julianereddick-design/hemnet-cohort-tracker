...
- **Monitored** (added 2026-08-14): `age-census-monthly` is registered in both `cron-health-slack.js` (the deployed daily monitor) and `cron-health.js` (the CLI) with a new `monthly` cadence — a 33-day window, since the longest healthy gap is 31 days. The shared query fetch was widened to 34/35 days so a monthly job's last run is visible at all. Until its first fire the spec carries `notBefore: '2026-09-02'`, rendering it as *deployed, first run due* rather than flagging daily; **delete that key once it has run once**. The monitor summarises the run as `N/4 pools persisted` and raises an issue when fewer than 4 persist or any pool's gates failed.
- **The report job is NOT in `cron_job_log`** — like its sibling `premarket-flow-weekly-report.js` it is not `runJob`-wrapped, so a failure of the 07:00 post is not monitored. The 02:00 measure job (the expensive, failure-prone half) is.
- **Verified 2026-08-14 without scraping:** `DRY_RUN=1 REPORT_DATE=… node age-census-report.js` on the droplet exercises the real DB read path and renders `MISSING` for all four pools while no data exists. It DOES write the two artifact files, so delete them afterwards if you dry-run a future date.
