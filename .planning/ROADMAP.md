...
- [~] 10-02: cosmetic-warning retargets + small consistency fixes — PARTIAL. Shipped 2026-05-26: (a/b) removed `oxylabsFallbackRate > 0.30` warning from Jobs A/C/D — rate now a reporting field, not an alert; (c) lowered Job B match-rate threshold 50%→30% (in-range 40-55% no longer alerts); (d) fixed Job C `defaultWeekDate()` Sun off-by-one (Sun now returns this-week's Mon, not last-week's); (f) raised Booli fetch cohort concurrency 2→8 (Plan 09-01 hardening lock relaxed — three sibling scrape jobs already at conc 8; expect Sun wall-clock to drop ~4× from ~3h to ~45 min, no more `budgetExceeded`). Carry over to next session: (e) `agent_id` FK constraint resolution per 09-2.5 #6 (needs operator decision: drop FK / drop column / two-phase write); (g) mirror D-24 COALESCE-preserve hardening onto Job A (09-2.5 #9); (h) fix Job B match-log URL (09-2.5 #8); (i) add `LOWER(TRIM(street_address))` functional index (09-2.6 #2). Stretch item: 09-2.5 #7 postcode-mismatch loosening (could lift Job B writes 41→55%).
