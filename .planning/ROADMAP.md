...
- ✅ **v2.1 Self-hosted scraper hardening** — Phase 10 (cleanup of observation-week carry-forwards; production-stable) — code-complete 2026-06-12 (10-05 droplet crontab/table-drop steps operator-gated)
...
- [x] 10-05 — retire the old Pool & Flow droplet fan-out + Slack — **repo SHIPPED 2026-06-12, droplet steps operator-gated.** (b) deleted the four `.js` (`listing-gap-monitor`, `flow-monitor`, `pool-flow-report`, `generate-pool-flow-charts`) + `setup-chart-cron.sh` from the repo; `git grep` confirmed the only consumers of `listing_gap_weekly`/`listing_flow_weekly` were these four scripts. `weekly-view-report.js` STAYS. (d) the `:3800` server (`view-data-server.js`) is KEPT — it also serves `weekly-view-report.js`; only `pool-flow-dashboard.html` is retired. **Operator-gated prod steps (irreversible live changes, batched for one sign-off):** (a) remove the four Mon-09:00 crontab lines on the droplet (keep `weekly-view-report.js`); (c) `DROP TABLE listing_gap_weekly, listing_flow_weekly` (no remaining consumer); remove the stale `pool-flow-dashboard.html`. See `deploy-instructions.md` Pool & Flow retirement note.
...
| 10. Self-hosted scraper hardening | v2.1 | 5/5 | Complete (10-05 droplet steps operator-gated) | 2026-06-12 |
