...
carry_forward:
  ...
  - "09-2.5 #8 (cosmetic, Phase 10): Job B match-log URL constructed as /bostad/ — wrong for active FS listings (should be /annons/ or use booli_listing.url). Affects log readability only. Fold into 10-02."
  ...
  - "09-2.5 #9 (Phase 10 hardening, symmetry): Job A's discovery-metadata UPDATEs lack the D-24 COALESCE-preserve hardening Job D got. Low-risk consistency improvement. Fold into 10-02."
  ...
  - "09-2.6 #2 (Phase 10 perf): hemnet_listingv2 lacks functional index on LOWER(TRIM(street_address)). Blocks the fuller delta filter + cohort_unmatched diagnostic. `CREATE INDEX CONCURRENTLY hemnet_listingv2_norm_street_idx ON hemnet_listingv2 (LOWER(TRIM(street_address)))`. Closes this + 09-2.5 #4. Fold into 10-02 or its own 10-05 if Phase 10 stays tight."
  ...
  - "09-1.5 #1 (Phase 10): 180-min JOB_BUDGET_MS for Job C may not fit full queue. budgetExceeded=true seen on id=438 (Sun 2026-05-24). Bump to 300 min or raise conc, per 09-02's Job D pattern (2→8). Fold into 10-02."
